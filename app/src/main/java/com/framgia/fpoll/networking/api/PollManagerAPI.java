...
public interface PollManagerAPI {
    ...
    Call<ResponseItem<List<String>>> updateLinkPoll(@Header("Authorization") String token,
            @Query("oldLinkUser") String oldUser, @Query("oldLinkAdmin") String oldAdmin,
            @Query("newLinkUser") String newUser, @Query("newLinkAdmin") String newAdmin);

    @GET("/api/v1/link/{token}")
    Call<ResponseItem<DataInfoItem>> getPoll(@Path("token") String token);

    @GET("/api/v1/poll-info/{token}")
    Call<ResponseItem<DataInfoItem>> getPollDetail(@Path("token") String token);
